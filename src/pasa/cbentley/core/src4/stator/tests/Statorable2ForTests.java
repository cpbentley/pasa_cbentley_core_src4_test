...
public class Statorable2ForTests extends StatorableAbstractForTests {
   ...
   public void stateReadFrom(StatorReader state) {
      ...
      state1 = (Statorable1ForTests) state.dataReadObject(tsc, state1);
      //use this if interface or sub implementation
      fancyStuff = (IFancyStuffTest) state.dataReadObject(tsc, fancyStuff);
   }

   public void stateWriteTo(StatorWriter state) {
      state.getWriter().writeString(name);
      state.dataWriterToStatorable(state1);
      state.dataWriterToStatorable(fancyStuff);
   }
